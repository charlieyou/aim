...
- Read-only for usage calls; credential files are updated only when a token refresh succeeds
- Token refresh occurs only after a 401 response when a refresh token is available
...
```
│ Claude               │ ⚠ No credential files found matching ~/.cli-proxy-api/claude-*.json │
...
- Claude credentials are read from `$HOME/.cli-proxy-api/claude-{email}.json`
  - JSON structure: `{ "access_token": "...", "refresh_token": "...", "type": "claude", ... }`
...
- Credential files are only modified when a token refresh succeeds
